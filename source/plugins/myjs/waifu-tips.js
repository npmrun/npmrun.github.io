...
function loadWidget(config) {
	...
	(function registerEventListener() {
		...
		document.querySelector("#waifu-tool .fa-times").addEventListener("click", () => {
			...
			document.getElementById("waifu").style.opacity = "0";
			setTimeout(() => {
	...
	let dx = 0
	let dy = 0
	let dw = waifu.offsetWidth
	let dh = waifu.offsetHeight
	// console.log(waifu.offsetLeft,waifu.offsetTop);
	setTimeout(() => {
		document.getElementById("waifu").style.opacity = 1;
	}, 1000);
	let downPositon;
	let width = window.innerWidth
	let height = window.innerHeight
	let isClick = false;
	let  left = localStorage.getItem("live2d_x")
	let  top = localStorage.getItem("live2d_y")
	if (left) {
	...
	waifu.addEventListener("mousedown",function(e){
		...
		dx =waifu.offsetLeft
		dy =waifu.offsetTop
		// dh =waifu.offsetHeight
		// dw =waifu.offsetWidth
		downPositon = {
			x:e.x,
			y:e.y
		}
	})
	waifu.addEventListener("mousemove",function(e){
		if (isClick) {
			let currPosition = {
				x:e.x,
				y:e.y
			}
			let left = (currPosition.x - (downPositon.x-dx));
			if (left<0) {
				left = 0;
			}
			if (left>(width- dw)){
			...
			let top = (currPosition.y - (downPositon.y-dy));
			if (top<0) {
				top = 0;
			}
			if (top>(height- dh)){
	...
	waifu.addEventListener("mouseup",function(e){
		isClick= false;
		localStorage.setItem("live2d_x",waifu.style.left)
		localStorage.setItem("live2d_y",waifu.style.top)
	})
}
